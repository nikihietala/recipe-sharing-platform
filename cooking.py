...
def check_if_ingredient_exists(ingredient_name):
    ...
    result = db.session.execute(
        text(sql), {
            "ingredient_name": ingredient_name}).fetchone()
...
def add_comment(recipe_id, content, poster_name):
    sql = """
    INSERT INTO comments (recipe_id, content, poster_name)
    VALUES (:recipe_id, :content, :poster_name)
    """
...
def search_recipes(ingredient=None, max_price=None, min_price=None):
    base_sql = """
    SELECT id, description, price, poster_name
    FROM recipes
    WHERE 1=1
    """
    params = {}
    ...
    if ingredient:
        ...
        AND EXISTS (
            ...
            AND LOWER(i.ingredient_name) = LOWER(:ingredient)
        )
        """
        base_sql += ingredient_sql
        params["ingredient"] = ingredient.strip()

    if max_price:
        max_price_sql = " AND price <= :max_price"
        base_sql += max_price_sql
        params["max_price"] = max_price

    if min_price:
        min_price_sql = " AND price >= :min_price"
        base_sql += min_price_sql
        params["min_price"] = min_price

    return db.session.execute(text(base_sql), params).fetchall()
...
def get_user_recipes(user_name):
    ...
    SELECT id, description, price, poster_name
    FROM recipes
    WHERE poster_name = :poster_name
    """
    recipes = db.session.execute(text(sql),
                                 {"poster_name": user_name}).fetchall()
    return recipes
